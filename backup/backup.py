...
def parse_arguments():
    if len(sys.argv) != 5:
        raise Exception("Usage: python backup.py <src> <dest> <backup_type> <backup_name>")
    src = sys.argv[1]
    dest = sys.argv[2]
    ...
    if not is_format_correct(backup_type):
        raise Exception("[ERROR] Format is not valid!")
...
def make_zip_folder():
    ...
    print(f"[OK] Backup compressed to {format_dest}")
